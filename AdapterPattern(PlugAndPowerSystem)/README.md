...
- **Refrigerator** – Represents a refrigerator device that requires a power source.  
...
- **LaptopAdapter** – An adapter for plugging a laptop into a standard power outlet.  
  It adapts the Laptop to the PowerOutlet interface, translating `plugIn()` to `charge()`.

- **RefrigeratorAdapter** – An adapter for plugging a refrigerator into a standard power outlet.  
  It adapts the Refrigerator to the PowerOutlet interface, translating `plugIn()` to `startCooling()`.

- **SmartphoneAdapter** – An adapter for plugging a smartphone charger into a standard power outlet.  
